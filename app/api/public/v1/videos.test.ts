...
import { OPTIONS, GET } from '@/app/api/public/v1/videos/route';
import { NextRequest } from 'next/server';

describe('public videos route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('OPTIONS returns 204 with CORS headers', async () => {
    const req = new NextRequest('http://localhost/api/public/v1/videos', { method: 'OPTIONS' });
  ...
  it('GET returns 401 when API key validation fails', async () => {
    ...
    const req = new NextRequest('http://localhost/api/public/v1/videos');
    const res = await GET(req);
    expect(res.status).toBe(401);
  });

  it('GET returns 404 when channel not found', async () => {
