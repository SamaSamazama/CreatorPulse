...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const description = await generateOpenRouterCompletion(process.env.OPENROUTER_THUMBNAIL_MODEL || process.env.OPENROUTER_COACH_MODEL || "z-ai/glm-5-2", imagePrompt, "You are a thumbnail design expert. Return only a concise visual description.");
