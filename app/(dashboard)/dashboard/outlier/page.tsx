...
export default function OutlierPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/ml/forecast", { method: "POST" }),
      fetch("/api/ml/outlier", { method: "POST" }),
    ]).then(async ([f, o]) => {
      if (!f.ok) throw new Error("Forecast failed");
      if (!o.ok) throw new Error("Outlier detection failed");
      const forecastData = await f.json();
      const outlierData = await o.json();
